...
public class Merger {
    ...
    MergeConfig mergeSchema(Object mainObject, Object subObject) {
        ...
        // Array
        if (main.getMinItems() != null ) {
            if (sub.getMinItems() != null) {
                mainSchema.setMinItems(Math.max(main.getMinItems(), sub.getMinItems()));
            } else {
                mainSchema.setMinItems(main.getMinItems());
            }
        } else {
            mainSchema.setMinItems(sub.getMinItems());
        }

        if (main.getMaxItems() != null) {
            if (sub.getMaxItems() != null) {
                mainSchema.setMaxItems(Math.min(main.getMaxItems(), sub.getMaxItems()));
            } else {
                mainSchema.setMaxItems(main.getMaxItems());
            }
        } else {
            mainSchema.setMaxItems(sub.getMaxItems());
        }

        if (main.getUniqueItems() != null) {
            if (sub.getUniqueItems() != null) {
                mainSchema.setUniqueItems(main.getUniqueItems() || sub.getUniqueItems());
            } else {
                mainSchema.setUniqueItems(main.getUniqueItems());
            }
        } else {
            mainSchema.setUniqueItems(sub.getUniqueItems());
        }

        if (areAllNull(main.getPrefixItems(), main.getItems())){
            if (areAllNull(sub.getPrefixItems(), sub.getPrefixItems())){

            } else {
                mainSchema.setPrefixItems(main.getPrefixItems());
                mainSchema.setItems(main.getItems());
            }
        } else {
            mainSchema.setPrefixItems(sub.getPrefixItems());
            mainSchema.setPrefixItems(sub.getPrefixItems());
        }
